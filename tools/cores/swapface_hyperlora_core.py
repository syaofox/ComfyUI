...
def run_v2(char: str, input_file: str, output_file: str, sub_body: bool = True, expression_edit: bool = False,
    ...
    try:
        ...
        with Workflow():
            ...
            aligned_image, _, inverse_rotation_angle = SFAlignImageByFace(analysis_models, image, True, False, None)
            bounding_infos, crop_images, _, = SFFaceCutout(analysis_models, aligned_image, 0, 0.4000000000000001, 'sdxl', 1, 42, 0, 24, False)
            _, _, _, _, crop_image, bounding_info = SFExtractBoundingBox(bounding_infos, crop_images, 0)
            instantid = InstantIDModelLoader('ip-adapter.bin')
            faceanalysis = InstantIDFaceAnalysis('CUDA')
            control_net = ControlNetLoader(r'instantid\diffusion_pytorch_model.safetensors')
            lora, image2 = HyperLoRALoadCharLoRA('京京_幼_混合')
            model, clip, vae = CheckpointLoaderSimple(r'xl\dreamshaperXL_v21TurboDPMSDE.safetensors')
            model = HyperLoRAApplyLoRA(model, lora, 0.85)
            ...
            image3 = VAEDecode(latent, vae)
            occluder = SFOccluderLoader('xseg_3')
            _, inverted_mask, _ = SFGeneratePreciseFaceMask(occluder, crop_image, 0.1, None)
            inverted_mask, _ = SFMaskChange(inverted_mask, 0, 0, False, 4, False)
            image4 = ImageCompositeMasked(image3, crop_image, 0, 0, False, inverted_mask)
            image5 = Canny(image4, 0.10000000000000002, 0.20000000000000004)
            positive2, negative2 = ControlNetApplyAdvanced(positive, negative, control_net2, image5, 0.8000000000000002, 0, 1, vae)
            latent2 = VAEEncode(crop_image, vae)
            mask_params = SFMaskParams(0, 0.07, False, 15, False)
            mask, _, _ = SFGeneratePreciseFaceMask(occluder, crop_image, 0.1, mask_params)
            latent2 = SetLatentNoiseMask(latent2, mask)
            ...
            image7 = SFImageColorMatch(image6, image6, 'LAB', 1, 'auto', 0, None)
            image8, _ = SFFacePaste(bounding_info, image7, aligned_image)
